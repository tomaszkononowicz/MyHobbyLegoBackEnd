...
<body>
        ...
        <ul>
            ...
            <li id="nav4"><a href="galery" class="selected">Galeria</a>
                <ul>
                    ...
                    <li id="nav42"><a href="galery_selected" class="selected">Zapamiętane</a></li>
                </ul>
            </li>
    ...
    <?php if ($galery->count()): ?>
		...
		<section class="galery_menu">
			<input class="button" type="submit" value="Zapomnij wybrane"/>
		</section>
        <?php foreach ($galery as $photo): ?>
			<?php if ( in_array($photo['_id'], $user['selected']) ): ?>
			<section class="photo">
				...
				<div class="data">
					...
					<?php endif ?>
					<div class="select">Zapamiętaj: <input type="checkbox" name="disselect[]" value="<?= $photo['_id'] ?>" /></div>
				</div>
			</section>
			<?php endif ?>
        <?php endforeach ?>
